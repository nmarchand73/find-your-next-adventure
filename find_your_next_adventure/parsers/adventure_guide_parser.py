...
from find_your_next_adventure.utils.maps import generate_google_maps_link, generate_extended_links
import fitz  # PyMuPDF
import ollama
...
class AdventureGuideParser:
    ...
    COUNTRY_MAPPING = {
        ...
        "CONGO": {"country": "Democratic Republic of Congo", "region": "Central Africa"},
        "ANGOLA": {"country": "Angola", "region": "Central Africa"},
        "EQUATORIAL GUINEA": {"country": "Equatorial Guinea", "region": "Central Africa"},
        ...
        "FRENCH POLYNESIA": {"country": "French Polynesia", "region": "Pacific Islands"},
    ...
    SPECIAL_CASES = {
        ...
        "NEW YORK, PARIS, OR LONDON": {"country": "Multiple", "region": "Global Cities"},
    ...
    CHAPTERS = {
        1: {"title": "From 90° North to 60° North", "range": {"from": "90° North", "to": "60° North"}, "ids": (1, 44)},
        2: {"title": "From 60° North to 45° North", "range": {"from": "60° North", "to": "45° North"}, "ids": (45, 265)},
        3: {"title": "From 45° North to 30° North", "range": {"from": "45° North", "to": "30° North"}, "ids": (266, 560)},
        4: {"title": "From 30° North to 15° North", "range": {"from": "30° North", "to": "15° North"}, "ids": (561, 711)},
        5: {"title": "From 15° North to 0° North", "range": {"from": "15° North", "to": "0° North"}, "ids": (712, 808)},
        6: {"title": "From 0° South to 15° South", "range": {"from": "0° South", "to": "15° South"}, "ids": (809, 861)},
        7: {"title": "From 15° South to 30° South", "range": {"from": "15° South", "to": "30° South"}, "ids": (862, 929)},
        8: {"title": "From 30° South to 90° South", "range": {"from": "30° South", "to": "90° South"}, "ids": (930, 1000)}
    }
    ...
    def __init__(self):
        ...
        self.stats = {"processed": 0, "successful": 0, "failed": 0, "unknown_countries": 0}
    ...
    def parse_coordinates(self, lat: str, lat_dir: str, lng: str, lng_dir: str) -> Coordinates:
        latitude = float(lat)
        longitude = float(lng)
    ...
    def identify_country_region(self, location: str) -> Tuple[str, str]:
        ...
        if ',' in location_upper:
            ...
            potential_country = re.sub(r'\b(PROVINCE|REGION|STATE|TERRITORY|GOVERNORATE)\b', '', potential_country).strip()
    ...
    def parse_pdf_content(self, content: str) -> Dict[int, List[Destination]]:
        ...
        logger.info(f"Processed: {self.stats['processed']}, Successful: {self.stats['successful']}, "
    ...
    def create_chapter_json(self, chapter_num: int, destinations: List[Destination]) -> Chapter:
    ...
    def create_combined_json(self, chapters_data: Dict[int, List[Destination]], output_dir: Path) -> None:
        combined_data = {
            "title": "Find Your Next Adventure - Complete Guide",
            "description": "Complete adventure destinations guide from 90° North to 90° South",
            "totalChapters": 8,
            "totalDestinations": sum(len(destinations) for destinations in chapters_data.values()),
            "chapters": {},
            "metadata": {
                "source": "Find Your Next Adventure Travel Guide",
                "generatedDate": "2025-07-31",
        ...
        try:
            ...
            for chapter_num, destinations in chapters_data.items():
                if destinations:
                    ...
                    output_file = output_dir / f"chapter_{chapter_num}_destinations.json"
