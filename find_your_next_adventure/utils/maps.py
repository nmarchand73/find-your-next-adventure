...
def clean_location_name(location: str) -> str:
    ...
    location = re.sub(r'^(START IN|START AT|START WITH|NEAR|ALL OVER|ACROSS|INCLUDES)\s+', '', location, flags=re.IGNORECASE)
    location = re.sub(r',?\s+(US|UK|USA|UNITED STATES|UNITED KINGDOM)$', '', location, flags=re.IGNORECASE)
...
def generate_google_street_view_link(location: str, coordinates: Coordinates, 
...
def generate_static_map_alternatives(location: str, coordinates: Coordinates,
                                    zoom: int = 15) -> Dict[str, str]:
    """
...
def generate_satellite_view_links(location: str, coordinates: Coordinates) -> Dict[str, str]:
    """
...
def generate_alternative_image_sources(location: str, coordinates: Coordinates) -> Dict[str, str]:
    """
...
def generate_panoramic_links(location: str, coordinates: Coordinates) -> Dict[str, str]:
    ...
    return {
        'google_street_view': generate_google_street_view_link(location, coordinates, fov=120),
...
def generate_location_image_links(location: str, coordinates: Coordinates, 
                                custom_zoom: int = 15,
                                detailed_view: bool = True) -> Dict[str, Union[str, Dict[str, str]]]:
    """
    ...
    result = {
        ...
        'static_maps': generate_static_map_alternatives(location, coordinates, custom_zoom),
    ...
    if detailed_view:
        result.update({
            ...
            'stock_photos': generate_alternative_image_sources(location, coordinates),
